...
class GameUI(tk.Frame):
    ...
    def draw_piece(self, index, first_player):
        ...
        if first_player:
            ...
            self.c.create_line(x, y, x+60, y+60, width=2.0, fill='#FFFFFF')
            self.c.create_line(x+60, y, x, y+60, width=2.0, fill='#FFFFFF')
    ...
    def on_draw(self):
        ...
        self.c.create_line(0,80,240,800,width=2.0, fill='#0077BB')
        self.c.create_line(0,160,240,160,width=2.0, fill='#0077BB')
        for i in range(9):
            if self.state.pieces[i] == 1:
                self.draw_piece(i, self.state.is_first_player())
            if self.state.enemy_pieces[i] == 1:
                self.draw_piece(i, not sel.state.is_first_player())
